...
struct school{
    ...
    func findTeacher(_ Id:Int)->Teacher?{
    ...
    func printFavoriteTeacherInfo(_ s:Student){
        if let teacher = findTeacher(s.favoriteTeacherIDproperty){
            if let email = teacher.email{
                print("Id:\(teacher.id) name:\(teacher.name) email:\(email) hair color:\(teacher.hairColor ?? "black")")
            }else{print("error")}
        }else{print("error")}
    }
}
...
let t1 = Teacher(name:"name",id: 3,hairColor: "black",email: nil)
let t2 = Teacher(name:"name",id: 3,hairColor: nil,email: nil)
let t3 = Teacher(name:"name",id: 3,hairColor: "black",email: "um@gmail.com")
let roster:[Teacher] = [t1,t2,t3]
...
*/
let daSchool = school(teachers: roster)
